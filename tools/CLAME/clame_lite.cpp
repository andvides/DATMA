...
bool readArguments(int argc,char *argv[], Args *args,Names *names, Parameters *parameters)
{
    ...
    while (argum < argc && not error) 
    {
        ...
        if ( IsParam(argv[argum],"-printOnlyNames") ) //print only names instead of fasta or fastq files 
        {
            argum ++;
            parameters->printOnlyNames = true;
            found1 = true;
            continue;
        }
            ...
            if(size>=parameters->sizeBin)
            {
                ...
                while(get!=stack2)
                {
                    ...
                    else
                    {
                        ...
                        if(parameters->fastq)
                        {
                            string quality=*(ptrQual+*(get)); 
                            myfile2<<'+'<<endl, myfile2<<quality<<endl;
                        }
                    }
                }
...
void printerror(const char arg[])
{
    ...
    cout << "  -printOnlyNames print bin output only Names (default false) " << endl;
